...
class Shell(GameObject):
    ...
    def move(self, time=1, grav=0):
        ...
        self.check_corners()
        if (
            self.vel[0] ** 2 + self.vel[1] ** 2 < 2 ** 2
            and self.coord[1] > SCREEN_SIZE[1] - 2 * self.rad
        ):
            self.is_alive = False
...
class Cannon(GameObject):
    ...
    def draw(self, screen):
        ...
        vec_1 = np.array(
            [
                int(5 * np.cos(self.angle - np.pi / 2)),
                int(5 * np.sin(self.angle - np.pi / 2)),
            ]
        )
        vec_2 = np.array(
            [int(self.pow * np.cos(self.angle)), int(self.pow * np.sin(self.angle))]
        )
